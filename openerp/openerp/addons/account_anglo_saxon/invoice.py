...
class account_invoice_line(osv.osv):
    ...
    def move_line_get(self, cr, uid, invoice_id, context=None):
        ...
        if inv.type in ('out_invoice','out_refund'):
            ...
            for i_line in inv.invoice_line:
                if i_line.product_id and i_line.product_id.valuation == 'real_time' and inv.type == 'in_invoice':
                    purchase_id = self.pool.get('purchase.order').search(cr, uid, [('name', '=', inv.origin)], context=context)
                    purchase = self.pool.get('purchase.order').browse(cr, uid, purchase_id, context=context)[0] or False
                    if i_line.product_id.type != 'service' and purchase.amount_total != inv.amount_total:
