...
class account_report_partner_statement(osv.osv):
...
#            search_tuples.extend(['|',('reconcile_id', '=', False), ('reconcile_id.create_date', '>', self._get_date(cr, uid, ids, context))])
        ...
        if fields['multicompany']:
            ...
            company_ids.extend(company_pool_obj.search(cr, uid, [('id', 'child_of', company_ids)] ))
...
### Function to prepare data for sending to report
    def check_report(self, cr, uid, ids, context=None):
        ...
        if isinstance(data['ids'], list):
            if len(data['ids'])>0:
                data['id'] = data['ids'][0]
        data['datex'] = self._get_date(cr, uid, ids, context)
        ...
        return {
            ...
            'report_name': 'partner.statement.new',
            'datas': data,
            'context': context,
            }
  
account_report_partner_statement()
