...
# 
    ...
    _columns = {
        ...
        'vehi_participants_ids':fields.many2many('transport.participant', 'vehicle_participant_rel', 'asset_id', 'student_id', 'Vehicle participants'),
    }
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False):
        ...
        return super(account_asset_asset, self).search(cr, uid, args, offset, limit, order, context, count)

account_asset_asset()

#class for participants
class transport_participant(osv.osv):
    
    _name = 'transport.participant'
    _rec_name = 'stu_pid_id'
    _description = 'Transport Participent Information'
    _columns = {
        'name': fields.many2one('student.student', 'Participent Name', required=True),
        'amount': fields.float('Amount', readonly=True),
        'transport_id': fields.many2one('student.transport', 'Transport Root', required=True),
        'stu_pid_id': fields.char('Personal Identification Number', size=50, required=True),
        'tr_reg_date': fields.date('Transportation Registrtion Date'),
        'tr_end_date': fields.date('Registrtion End Date'),
        'months': fields.integer('Registration For Months'),
        'asset_id': fields.many2one('account.asset.asset', 'vehicle No'),
...
class student_transport(osv.osv):
    ...
    _columns = {
        'name':fields.char('Transport Root Name', size=30, required=True),
        ...
        'trans_vehicle_ids':fields.many2many('account.asset.asset', 'transport_vehicle_rel', 'vehicle_id', 'transport_id', ' Vehicles'),
        'trans_point_ids':fields.many2many('transport.point', 'transport_point_rel', 'root_id', 'point_id', 'Points'),
...
class transport_registration(osv.osv):
    ...
    _columns = {
        ...
        'reg_state':fields.selection([('draft', 'Draft'),
                                      ('confirm', 'Confirm'),
                                      ('cancel', 'Cancel')
                                     ], 'State', readonly=True),
        'asset_id': fields.many2one('account.asset.asset', 'vehicle No', required=True),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self.write(cr, uid, ret_val, {'m_amount':m_amt and m_amt['value'] and m_amt['value'].get('m_amount',0.00) or 0.00, 'reg_end_date':ex_dt and ex_dt['value'] and ex_dt['value'].get('reg_end_date',None) or None})
        return ret_val
    ...
    def trans_regi_cancel(self, cr, uid, ids, context=None):
        ...
        @return : True 
        '''
        self.write(cr, uid, ids, {'reg_state' : 'cancel'}, context=context)
        return True
    ...
    def trans_regi_confirm(self, cr, uid, ids, context=None):
        ...
        self.write(cr, uid, ids, {'reg_state' : 'confirm'}, context=context)
        ...
        for reg_data in self.browse(cr, uid, ids, context=context):
            ...
            if reg_data.for_month <= 0:
                raise osv.except_osv(_('Warning !'), _('Registration months must be more than Zero!'))
            # First Check Is there vacancy or not
            person = int(reg_data.asset_id.participant) + 1
            if reg_data.asset_id.capacity < person:
                raise osv.except_osv(_('Warning !'), _('There is No More vacancy on this vehicle.'))
            ...
            if tr_end_date > date:
                raise osv.except_osv(_('Warning !'), _('For this much Months Registration is not Possible, Root end date is Early.'))
            ...
            temp = stu_prt_obj.create(cr, uid, {
                                        ...
                                        'asset_id': reg_data.asset_id.id,
                                        })
            
            #make entry in Transport vehicle. 
            list1 = []
            for prt in reg_data.asset_id.vehi_participants_ids:
                list1.append(prt.id)
            flag = True
            for prt in list1:
            ...
            if flag:
                list1.append(temp)
            vehi_obj.write(cr, uid, reg_data.asset_id.id, {'vehi_participants_ids':[(6, 0, list1)]}, context=context)
