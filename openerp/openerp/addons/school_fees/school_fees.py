...
class student_fees_register(osv.osv):
    """
            ...
            for stu in student_pool.browse(cr, uid, student_ids, context=context):
                ...
                if old_slips:
                    ...
                    for sid in old_slips:
                        wf_service.trg_validate(uid, 'student.payslip', sid, 'payslip_confirm', cr)
                else:
                    res = {
                        'student_id':stu.id,
                        'register_id':vals.id,
                    ...
                    wf_service.trg_validate(uid, 'student.payslip', slip_id, 'payslip_confirm', cr)
...
student_fees_register()

class student_payslip_line(osv.osv):
    '''
...
student_payslip_line()

class student_fees_structure_line(osv.osv):
    '''
...
student_fees_structure_line()

class student_fees_structure(osv.osv):
    """
    Fees structure
    """
...
#        'company_id': fields.many2one('res.company', 'Company', required=True, change_default=True, readonly=True, states={'draft':[('readonly', False)]}),
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for fees in self.browse(cr, uid, ids, context=context):
            ...
            company_currency = fees.school_id.currency_id.id
            ...
            if fees.type in ('in_invoice', 'out_refund'):
                ...
                comapny_ac_id = fees.school_id.partner_id.property_account_receivable.id
            elif fees.type in ('out_invoice', 'in_refund'):
                account_id = fees.student_id.property_account_receivable.id
                comapny_ac_id = fees.school_id.partner_id.property_account_payable.id
            ...
            ctx.update({'school_id': fees.school_id.id})
...
class student_payslip_line_line(osv.osv):
    '''
    Function Line
    '''
