...
class modified_budget_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(modified_budget_report, self).__init__(cr, uid, name, context=context)
    ...
    def funct(self, object, form, ids=None, done=None, level=1):
        ...
        acc_budget_post = self.pool.get('account.budget.post')
        for budget_id in budgets:
            ...
            self.cr.execute('SELECT DISTINCT(general_budget_id) FROM crossovered_budget_lines WHERE id = ANY(%s)',(budget_ids,))
            ...
            for i in range(0, len(an_ids)):
                if not an_ids[i][0]:
                    continue
                budget_name = acc_budget_post.browse(self.cr, self.uid, [an_ids[i][0]])
                res={
                    'b_id': '-1',
                    'a_id': '-1',
                ...
                line_ids = c_b_lines_obj.search(self.cr, self.uid, [('id', 'in', budget_ids), ('general_budget_id','=',an_ids[i][0])])
                ...
                for line in line_id:
                    if line.id in budget_ids:
                        ...
                        if line.analytic_account_id.id in done_budget:
                            ...
                            if form['report'] == 'analytic-full':
                                result.append(res1)
                                done_budget.append(line.analytic_account_id.id)
                    else:
                ...
                if form['report'] == 'analytic-full':
                    result[-(len(done_budget) +1)]['theo'] = abs(tot_theo)
                    tot['theo'] += tot_theo
                    result[-(len(done_budget) +1)]['pln'] = abs(tot_pln)
                    tot['pln'] += tot_pln
                    result[-(len(done_budget) +1)]['prac'] = abs(tot_prac)
                    tot['prac'] += tot_prac
                    result[-(len(done_budget) +1)]['perc'] = abs(tot_perc)
                else:
                    result[-1]['theo'] = abs(tot_theo)
                    tot['theo'] += tot_theo
                    result[-1]['pln'] = abs(tot_pln)
                    tot['pln'] += tot_pln
                    result[-1]['prac'] = abs(tot_prac)
                    tot['prac'] += tot_prac
                    result[-1]['perc'] = abs(tot_perc)
    ...
    def funct_total(self, form):
        ...
        res = {}
        res = {
             'tot_theo': abs(tot['theo']),
             'tot_pln': abs(tot['pln']),
             'tot_prac': abs(tot['prac']),
             'tot_perc': abs(tot['perc'])
        }
        result.append(res)
        return result

report_sxw.report_sxw('report.crossovered.budget.modified.report', 'crossovered.budget', 'addons/project_costing/report/modified_crossovered_budget_report.rml',parser=modified_budget_report,header='internal')
