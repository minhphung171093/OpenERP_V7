...
class binary_ir8s_text_file_wizard(osv.osv_memory):
    ...
    _columns = {
        ...
        'ir8s_txt_file': fields.binary('Click On Save As Button To Download File', readonly=True),
    }
            ...
            for contract in contract_obj.browse(cr, uid, contract_ids):
                ...
                if contract_income_tax_ids:
                    for emp in hr_contract_income_tax_obj.browse(cr, uid, [contract_income_tax_ids[0]]):
                        payslip_ids = payslip_obj.search(cr, uid, [('date_from', '>=', start_date), ('date_from', '<=', end_date), ('employee_id', '=', contract.employee_id.id)])
                        ...
                        fromdate = todate = add_wage_date = eyer_date = eyee_date = ''
                        if emp.fromdate:
...
#                        if not emp.eyer_contibution:
#                            raise osv.except_osv(_('Error'), _('Employer Contribution not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.eyee_contibution:
#                            raise osv.except_osv(_('Error'), _('Employee Contribution not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.additional_wage:
#                            raise osv.except_osv(_('Error'), _('Additional wage not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.refund_eyers_date:
#                            raise osv.except_osv(_('Error'), _('Refund Employer Date not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.refund_eyees_date:
#                            raise osv.except_osv(_('Error'), _('Refund Employee Date not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.refund_eyers_interest_contribution:
#                            raise osv.except_osv(_('Error'), _('Refund Employer Interest Contribution not found for %s employee.' % (contract.employee_id.name) ))
#                        if not emp.refund_eyees_interest_contribution:
#                            raise osv.except_osv(_('Error'), _('Refund Employee Interest Contribution not found for %s employee.' % (contract.employee_id.name) ))
