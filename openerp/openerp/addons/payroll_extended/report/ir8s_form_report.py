        ...
        for contract in contract_obj.browse(self.cr, self.uid, contract_ids):
            ...
            if contract_income_tax_ids:
                ...
                for emp in hr_contract_income_tax_obj.browse(self.cr, self.uid, [contract_income_tax_ids[0]]):
                    payslip_ids = payslip_obj.search(self.cr, self.uid, [('date_from', '>=', start_date), ('date_from', '<=', end_date), ('employee_id', '=', contract.employee_id.id)])
...
report_sxw.report_sxw('report.ppd_ir8s_form_report','hr.payslip','addons/payroll_extended/report/ir8s_form_report_tmpl.rml',parser=ppd_ir8s_form)
