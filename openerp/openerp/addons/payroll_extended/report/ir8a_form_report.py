    ...
    def get_data(self, form):
        ...
        for employee in employee_obj.browse(self.cr, self.uid, emp_ids):
            ...
            if contract_income_tax_ids:
                ...
                salary_amt = other_allowance = other_data = amount_data = mbf_amt = donation_amt = catemp_amt = net_amt = bonus_amt = prv_yr_gross_amt = gross_amt = gross_commission = 0
                prev_yr_payslip_ids = payslip_obj.search(self.cr, self.uid, [('date_from', '>=', prev_yr_start_date), ('date_from', '<=', prev_yr_end_date), ('employee_id', '=', employee.id)])
                for payslip in payslip_obj.browse(self.cr, self.uid, prev_yr_payslip_ids):
                    for line in payslip.line_ids:
                        if line.code == 'GROSS':
                            prv_yr_gross_amt += line.amount
                payslip_ids = payslip_obj.search(self.cr, self.uid, [('date_from', '>=', start_date), ('date_from', '<=', end_date), ('employee_id', '=', employee.id)])
...
#                        
                ...
                net_amt = net_amt
                bonus_amt = bonus_amt
                insurance = director_fee = gain_profit = exempt_income = benifits_in_kinds = gains_profit_share_option = excess_voluntary_contribution_cpf_employer = contribution_employer = retirement_benifit_from = retirement_benifit_up = compensation_loss_office = gratuity_payment_amt = entertainment_allowance = transport_allowance = pension = 0
...
#                transport_allowance = entertainment_allowance = 0.00
                ...
                res['catemp_amt'] = int(catemp_amt) * 1.00
                if insurance:
                    insurance = insurance%1 > 0.00 and (int(insurance) + 1) or insurance
                if gross_commission:
                    fromdate = '01/01/%s'%str(fromdate)
                    todate = '31/01/%s'%str(todate)
                else:
                    fromdate = ''
                    todate = ''
...
report_sxw.report_sxw('report.ppd_ir8a_form_report','hr.payslip','addons/payroll_extended/report/ir8a_form_report_tmpl.rml',parser=ppd_ir8a_form)
